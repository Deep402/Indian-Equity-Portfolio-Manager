...
def validate_ticker(ticker):
    try:
        stock = yf.Ticker(ticker)
        # Check if the ticker is valid by fetching its info
        stock.info
        return True
    except:
        return False
...
def display_combined_dashboard(portfolios):
    ...
    print("{:<20} {:<15} {:<15} {:<15} {:<15}".format(
        "Portfolio", "Invested", "Current", "Profit/Loss", "Profit/Loss %"
    ))
    print("-" * 80)
    ...
    for portfolio_name, portfolio in portfolios.items():
        ...
        if portfolio_profit_loss >= 0:
            profit_loss_color = Fore.GREEN
        else:
            profit_loss_color = Fore.RED

        print("{:<20} ₹{:<14,.2f} ₹{:<14,.2f} {}{:<14,.2f}{} {}{:<15.2f}%{}".format(
            portfolio_name, portfolio_investment, portfolio_current_value,
            profit_loss_color, portfolio_profit_loss, Style.RESET_ALL,
            profit_loss_color, portfolio_profit_loss_percent, Style.RESET_ALL
        ))
    ...
    if total_profit_loss >= 0:
        total_profit_loss_color = Fore.GREEN
    else:
        total_profit_loss_color = Fore.RED

    print("\n--- Combined Metrics ---")
    print(f"Total Invested: ₹{total_investment:,.2f}")
    print(f"Total Current Value: ₹{total_current_value:,.2f}")
    print(f"Total Profit/Loss: {total_profit_loss_color}₹{total_profit_loss:,.2f} ({total_profit_loss_percent:.2f}%){Style.RESET_ALL}")
...
def display_individual_dashboard(portfolios):
    ...
    print(f"Total Invested: ₹{total_investment:,.2f}")
    print(f"Total Current Value: ₹{total_current_value:,.2f}")
    print(f"Total Profit/Loss: {profit_loss_color}₹{total_profit_loss:,.2f} ({total_profit_loss_percent:.2f}%){Style.RESET_ALL}")
...
def add_stock(portfolios):
    ...
    while True:
        ...
        if validate_date(purchase_date):  # Validate the date format
            break
        else:
...
def export_individual_portfolio(portfolios):
    ...
    print(f"Portfolio '{portfolio_name}' exported to '{file_name}'.")
...
def export_all_portfolios(portfolios):
    ...
    print("All portfolios exported to 'all_portfolios.xlsx'.")
...
def main():
    ...
    try:
        while True:
            ...
            print("7. Export Individual Portfolio to Excel")
            print("8. Export All Portfolios to Excel")
            print("9. Exit")
            choice = input("Enter your choice: ")
