...
class MarketDataFetcher:
    def __init__(self):
        ...
        self._base_delay = 3  # Base delay in seconds
        self._max_delay = 30  # Maximum delay in seconds
        
    def _get_delay(self, ticker):
        with self._lock:
            current_time = time.time()
            if ticker in self._last_request_time:
                time_since_last = current_time - self._last_request_time[ticker]
                request_count = self._request_count.get(ticker, 0)
                
                # Exponential backoff with jitter
                delay = min(self._base_delay * (2 ** request_count), self._max_delay)
    ...
    def _update_request_stats(self, ticker):
        with self._lock:
            self._last_request_time[ticker] = time.time()
            self._request_count[ticker] = self._request_count.get(ticker, 0) + 1
            
    def _reset_request_stats(self, ticker):
        with self._lock:
            self._request_count[ticker] = 0
            
    def _process_queue(self):
        while True:
            try:
                ticker, callback = self.queue.get()
                if ticker is None:  # Poison pill
                    break
                    
                delay = self._get_delay(ticker)
                if delay > 0:
                    time.sleep(delay)
                ...
                try:
                    ...
                    self._reset_request_stats(ticker)  # Reset on success
                    callback(ticker, hist)
                except Exception as e:
                    if "Too Many Requests" in str(e):
                        self._update_request_stats(ticker)
                        # Requeue the request with exponential backoff
                        self.queue.put((ticker, callback))
                        time.sleep(self._get_delay(ticker))
                    else:
    ...
    def fetch_price(self, ticker, callback):
        self.queue.put((ticker, lambda t, h: callback(t, h['Close'].iloc[-1] if h is not None and not h.empty else None)))
        
    def fetch_batch(self, tickers, callback):
        results = {}
        remaining = len(tickers)
        
        def batch_callback(ticker, data):
            nonlocal remaining
        ...
        for ticker in tickers:
            self.fetch_price(ticker, lambda t, p: batch_callback(t, p))
...
class MarketDataWorker(QThread):
    ...
    def run(self):
        ...
        def update_results(ticker_data):
            nonlocal remaining
        ...
        # Fetch all tickers in batch
        self.fetcher.fetch_batch(list(self.indices.values()), update_results)
...
class PortfolioTracker(QMainWindow):
    ...
    def plot_sector_chart(self, data):
