...
def show_market_snapshot():
    ...
    try:
        ...
        snapshot.add_row(
            f"[{THEME['primary']}]Nifty 50:[/] [bold {THEME['success']}]₹{prices.get('^NSEI', 0):,.2f}[/]",
            f"[{THEME['primary']}]S&P 500:[/] [bold {THEME['success']}]${prices.get('^GSPC', 0):,.2f}[/]"
        )
        snapshot.add_row(
            f"[{THEME['primary']}]Sensex:[/] [bold {THEME['success']}]₹{prices.get('^BSESN', 0):,.2f}[/]",
...
def display_individual_dashboard(portfolio_name, portfolio):
    ...
    try:
        ...
        header = Panel(
            Text.assemble(
                (f"🏦 {portfolio_name.upper()} ", f"bold {THEME['primary']}"),
                ("PORTFOLIO", "bold")
            ),
        ...
        console.print(f"[{THEME['primary']}]{'━' * 130}[/]\n")
        ...
        columns = [
            ...
            ("Price (₹)", THEME['primary'], 15),
            ("Today %", THEME['warning'], 12),
            ("Today P/L (₹)", THEME['warning'], 18),
            ("Total P/L (₹)", THEME['info'], 38)  # Significantly increased width
        ]
        ...
        for _, row in portfolio.iterrows():
            ...
            total_pl_str = f"₹{row['Profit/Loss']:+,.2f} ({row['Profit/Loss %']:+.2f}%)"
...
def display_combined_dashboard(portfolios):
    ...
    try:
        ...
        if not portfolios:
            return Panel(
                "[bold red]⚠️ No portfolios found. Create one first![/]", 
                border_style=THEME['danger'],
                style=f"on {THEME['background']}"
            )
        ...
        def format_currency(value):
            ...
            num_str = f"₹{abs(value):,.2f}" if value >= 0 else f"-₹{abs(value):,.2f}"
        ...
        console.print(header)
        console.print(f"[{THEME['primary']}]{'━' * 80}[/]\n")
        ...
        columns = [
            ...
            ("Invested (₹)", THEME['success'], 20),
            ("Current (₹)", THEME['primary'], 20),
            ("Total P/L (₹)", THEME['info'], 18),
            ("Today P/L (₹)", THEME['warning'], 18),
...
def plot_portfolio_allocation(portfolio, portfolio_name):
    ...
    try:
        ...
        fig.update_traces(
            ...
            hovertemplate="<b>%{label}</b><br>%{percent:.1%}<br>₹%{value:,.2f}"
        )
...
def plot_profit_loss(portfolio, portfolio_name):
    ...
    try:
        ...
        fig = px.bar(
            ...
            text=[f"₹{x:+,.2f}" for x in portfolio['Profit/Loss']]
        )
        ...
        fig.update_layout(
            ...
            yaxis=dict(title="Profit/Loss (₹)", showgrid=False),
...
def main_menu():
    ...
    while True:
        try:
            choice = input(f"\n[bold {THEME['primary']}]» Select option (1-7): [/]").strip()
