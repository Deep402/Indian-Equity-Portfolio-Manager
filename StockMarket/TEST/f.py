...
# Function to refresh the combined dashboard every 10 seconds
def refresh_combined_dashboard(portfolios):
    try:
        while True:
            display_combined_dashboard(portfolios)
            time.sleep(10)  # Refresh every 10 seconds
    except KeyboardInterrupt:
        console.print("\n[bold green]Stopped refreshing combined dashboard.[/bold green]")

# Function to refresh the individual dashboard every 10 seconds
def refresh_individual_dashboard(portfolios, portfolio_name):
    try:
        while True:
            display_individual_dashboard(portfolios, portfolio_name)
            time.sleep(10)  # Refresh every 10 seconds
    except KeyboardInterrupt:
        console.print("\n[bold green]Stopped refreshing individual dashboard.[/bold green]")
...
def display_combined_dashboard(portfolios):
    ...
    # Plot combined portfolio allocation
    plot_combined_portfolio_allocation(portfolios)

    # Plot combined profit/loss by stock
...
def manage_shares(portfolios):
    ...
    # Display existing shares with numbers
    ...
    while True:
        try:
            stock_choice = int(input("Enter the number corresponding to the stock: "))
...
def main():
    ...
    try:
        while True:
            ...
            console.print("6. View Combined Dashboard (Auto Refresh)")
            console.print("7. View Individual Dashboard (Auto Refresh)")
            ...
            if choice == "1":
                ...
                refresh_combined_dashboard(portfolios)
            elif choice == "7":
                portfolio_name = select_portfolio(portfolios)
                if portfolio_name:
                    refresh_individual_dashboard(portfolios, portfolio_name)
            elif choice == "8":
